...
static gdouble dmsh2rad(const gchar *latlon) {
  ...
  if (deg > 180 || min >= 60 || sec >= 60) return DBL_MAX;
...
gboolean weather_location_equal(const WeatherLocation *location1,
  ...
  if (location1 == NULL || location2 == NULL) return (location1 == location2);
...
const gchar *weather_wind_direction_string(WeatherWindDirection wind) {
  if (wind <= WIND_INVALID || wind >= WIND_LAST) return _("Invalid");
...
const gchar *weather_sky_string(WeatherSky sky) {
  if (sky <= SKY_INVALID || sky >= SKY_LAST) return _("Invalid");
...
gboolean requests_init(WeatherInfo *info) {
  if (info->requests_pending) return FALSE;
...
void request_done(WeatherInfo *info, gboolean ok) {
  ...
  if (!--info->requests_pending) info->finish_cb(info, info->cb_data);
}
...
void free_forecast_list(WeatherInfo *info) {
  ...
  for (p = info->forecast_list; p; p = p->next) weather_info_free(p->data);
...
WeatherInfo *_weather_info_fill(WeatherInfo *info, WeatherLocation *location,
  ...
  if (!info) {
    ...
    if (info->forecast) g_free(info->forecast);
    info->forecast = NULL;
...
WeatherInfo *weather_info_clone(const WeatherInfo *info) {
  ...
  if (clone->radar != NULL) g_object_ref(clone->radar);
...
void weather_info_free(WeatherInfo *info) {
  ...
  if (info->session) g_object_unref(info->session);
...
const gchar *weather_info_get_sky(WeatherInfo *info) {
  g_return_val_if_fail(info != NULL, NULL);
  if (!info->valid) return "-";
  if (info->sky < 0) return _("Unknown");
    ...
    case TEMP_UNIT_FAHRENHEIT:
      if (!want_round) {
        ...
        if (fetestexcept(range_problem))
          ...
          /* Translators: This is the temperature in degrees Fahrenheit
           * (\302\260 is U+00B0 DEGREE SIGN) */
    ...
    case TEMP_UNIT_CENTIGRADE:
      if (!want_round) {
        ...
        if (fetestexcept(range_problem))
          ...
          /* Translators: This is the temperature in degrees Celsius (\302\260
           * is U+00B0 DEGREE SIGN) */
...
const gchar *weather_info_get_temp(WeatherInfo *info) {
  ...
  if (!info->valid) return "-";
  if (info->temp < -500.0) return _("Unknown");
...
const gchar *weather_info_get_temp_min(WeatherInfo *info) {
  ...
  if (!info->valid || !info->tempMinMaxValid) return "-";
  if (info->temp_min < -500.0) return _("Unknown");
...
const gchar *weather_info_get_temp_max(WeatherInfo *info) {
  ...
  if (!info->valid || !info->tempMinMaxValid) return "-";
  if (info->temp_max < -500.0) return _("Unknown");
...
const gchar *weather_info_get_dew(WeatherInfo *info) {
  ...
  if (!info->valid) return "-";
  if (info->dew < -500.0) return _("Unknown");
...
const gchar *weather_info_get_humidity(WeatherInfo *info) {
  ...
  if (humidity < 0.0) return _("Unknown");
...
const gchar *weather_info_get_apparent(WeatherInfo *info) {
  ...
  if (apparent < -500.0) return _("Unknown");
...
static const gchar *windspeed_string(gfloat knots, SpeedUnit to_unit) {
  ...
  switch (to_unit) {
    ...
    case SPEED_UNIT_MPH:
      ...
      g_snprintf(buf, sizeof(buf), _("%.1f mph"),
                 WINDSPEED_KNOTS_TO_MPH(knots));
      break;
    case SPEED_UNIT_KPH:
      /* Translators: This is the wind speed in kilometers per hour */
      g_snprintf(buf, sizeof(buf), _("%.1f km/h"),
                 WINDSPEED_KNOTS_TO_KPH(knots));
      break;
    case SPEED_UNIT_MS:
...
const gchar *weather_info_get_wind(WeatherInfo *info) {
  ...
  if (!info->valid) return "-";
  if (info->windspeed < 0.0 || info->wind < 0) return _("Unknown");
...
const gchar *weather_info_get_pressure(WeatherInfo *info) {
  ...
  if (!info->valid) return "-";
  if (info->pressure < 0.0) return _("Unknown");
...
const gchar *weather_info_get_visibility(WeatherInfo *info) {
  ...
  if (!info->valid) return "-";
  if (info->visibility < 0.0) return _("Unknown");
  ...
  switch (info->distance_unit) {
    ...
    default:
      g_warning("Conversion to illegal visibility unit: %d",
                info->pressure_unit);
      return _("Unknown");
  }
...
const gchar *weather_info_get_sunrise(WeatherInfo *info) {
  ...
  if (!info->location->latlon_valid) return "-";
  if (!info->valid) return "-";
  if (!calc_sun(info)) return "-";

  localtime_r(&info->sunrise, &tm);
  if (strftime(buf, sizeof(buf), _("%H:%M"), &tm) <= 0) return "-";
  return buf;
}
...
const gchar *weather_info_get_temp_summary(WeatherInfo *info) {
  ...
  if (!info->valid || info->temp < -500.0) return "--";
...
gchar *weather_info_get_weather_summary(WeatherInfo *info) {
  ...
  if (!info->valid) return g_strdup(_("Retrieval failed"));
  buf = weather_info_get_conditions(info);
  if (!strcmp(buf, "-")) buf = weather_info_get_sky(info);
...
static gboolean temperature_value(gdouble temp_f, TempUnit to_unit,
  ...
  *value = 0.0;
  if (temp_f < -500.0) return FALSE;

  if (to_unit == TEMP_UNIT_DEFAULT) to_unit = def_unit;
...
static gboolean speed_value(gdouble knots, SpeedUnit to_unit, gdouble *value,
  ...
  *value = -1.0;
  ...
  if (knots < 0.0) return FALSE;

  if (to_unit == SPEED_UNIT_DEFAULT) to_unit = def_unit;
...
static gboolean pressure_value(gdouble inHg, PressureUnit to_unit,
  ...
  *value = -1.0;
  ...
  if (inHg < 0.0) return FALSE;

  if (to_unit == PRESSURE_UNIT_DEFAULT) to_unit = def_unit;
...
static gboolean distance_value(gdouble miles, DistanceUnit to_unit,
  ...
  *value = -1.0;
  ...
  if (miles < 0.0) return FALSE;

  if (to_unit == DISTANCE_UNIT_DEFAULT) to_unit = def_unit;
...
gboolean weather_info_get_value_sky(WeatherInfo *info, WeatherSky *sky) {
  ...
  if (info->sky <= SKY_INVALID || info->sky >= SKY_LAST) return FALSE;
...
gboolean weather_info_get_value_conditions(
    WeatherInfo *info, WeatherConditionPhenomenon *phenomenon,
  ...
  if (!info->cond.significant) return FALSE;
...
gboolean weather_info_get_value_temp_min(WeatherInfo *info, TempUnit unit,
  ...
  if (!info->valid || !info->tempMinMaxValid) return FALSE;
...
gboolean weather_info_get_value_sunrise(WeatherInfo *info, time_t *value) {
  ...
  if (!info->valid || !info->sunriseValid) return FALSE;
...
gboolean weather_info_get_value_sunset(WeatherInfo *info, time_t *value) {
  ...
  if (!info->valid || !info->sunsetValid) return FALSE;
...
gboolean weather_info_get_value_moonphase(WeatherInfo *info,
  ...
  if (!info->valid || !info->moonValid) return FALSE;

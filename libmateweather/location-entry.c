...
static void mateweather_location_entry_build_model(
    MateWeatherLocationEntry *entry, MateWeatherLocation *top);
...
static void finalize(GObject *object) {
  ...
  if (entry->location) mateweather_location_unref(entry->location);
  if (entry->top) mateweather_location_unref(entry->top);
  ...
  switch (prop_id) {
    ...
    case PROP_LOCATION:
      mateweather_location_entry_set_location(
          MATEWEATHER_LOCATION_ENTRY(object), g_value_get_pointer(value));
      break;
    default:
 ...
 **/
gboolean mateweather_location_entry_has_custom_text(
    MateWeatherLocationEntry *entry) {
...
static void fill_location_entry_model(GtkTreeStore *store,
  ...
  switch (mateweather_location_get_level(loc)) {
    ...
    case MATEWEATHER_LOCATION_ADM1:
      ...
      for (i = 0; children[i]; i++) {
        fill_location_entry_model(store, children[i], display_name,
                                  compare_name);
      }
    ...
    case MATEWEATHER_LOCATION_CITY:
      if (children[0] && children[1]) {
        ...
        for (i = 0; children[i]; i++) {
          ...
          gtk_tree_store_set(
              store, &iter, MATEWEATHER_LOCATION_ENTRY_COL_LOCATION,
              children[i], MATEWEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME,
              display_name, MATEWEATHER_LOCATION_ENTRY_COL_COMPARE_NAME,
              compare_name, -1);
...
static char *find_word(const char *full_name, const char *word, int word_len,
  ...
  while ((p = strchr(p + 1, *word))) {
    if (strncmp(p, word, word_len) != 0) continue;

    if (p > (char *)full_name) {
      char *prev = g_utf8_prev_char(p);

      /* Make sure p points to the start of a word */
      if (g_unichar_isalpha(g_utf8_get_char(prev))) continue;

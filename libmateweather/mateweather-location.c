...
static gboolean parse_coordinates(const char *coordinates, double *latitude,
  ...
  *latitude = g_ascii_strtod(coordinates, &p) * M_PI / 180.0;
  if (p == (char *)coordinates) return FALSE;
  if (*p++ != ' ') return FALSE;
...
static MateWeatherLocation *location_new_from_xml(
    MateWeatherParser *parser, MateWeatherLocationLevel level,
  ...
  if (xmlTextReaderRead(parser->xml) != 1) goto error_out;
  while ((tagtype = xmlTextReaderNodeType(parser->xml)) !=
    ...
    if (!strcmp(tagname, "name") && !loc->name) {
      ...
      if (!loc->zones) goto error_out;

    } else {
      if (xmlTextReaderNext(parser->xml) != 1) goto error_out;
    }
  }
  if (xmlTextReaderRead(parser->xml) != 1 && parent) goto error_out;
...
void mateweather_location_unref(MateWeatherLocation *loc) {
  ...
  if (loc->zones) {
    for (i = 0; loc->zones[i]; i++) mateweather_timezone_unref(loc->zones[i]);
    g_free(loc->zones);
  }
 ...
 **/
MateWeatherLocationLevel mateweather_location_get_level(
 ...
 **/
MateWeatherLocation **mateweather_location_get_children(
    MateWeatherLocation *loc) {
...
const char *mateweather_location_get_country(MateWeatherLocation *loc) {
  ...
  while (loc->parent && !loc->country_code) loc = loc->parent;
 ...
 * %NULL
 **/
MateWeatherTimezone *mateweather_location_get_timezone(
    MateWeatherLocation *loc) {
  const char *tz_hint;
  int i;
  ...
  while (loc && !loc->tz_hint) loc = loc->parent;
  if (!loc) return NULL;
  tz_hint = loc->tz_hint;

  while (loc) {
    while (loc && !loc->zones) loc = loc->parent;
    if (!loc) return NULL;
...
static void add_timezones(MateWeatherLocation *loc, GPtrArray *zones) {
  ...
  if (loc->level < MATEWEATHER_LOCATION_COUNTRY && loc->children) {
    for (i = 0; loc->children[i]; i++) add_timezones(loc->children[i], zones);
  }
}
 ...
 **/
MateWeatherTimezone **mateweather_location_get_timezones(
    MateWeatherLocation *loc) {
  GPtrArray *zones;
...
void mateweather_location_free_timezones(MateWeatherLocation *loc,
  ...
  for (i = 0; zones[i]; i++) mateweather_timezone_unref(zones[i]);
  g_free(zones);
}
...
WeatherLocation *mateweather_location_to_weather_location(
    MateWeatherLocation *gloc, const char *name) {
  ...
  if (!name) name = mateweather_location_get_name(gloc);
  ...
  while (l && (!code || !zone || !radar || !tz_hint)) {
    if (!code && l->station_code) code = l->station_code;
    if (!zone && l->forecast_zone) zone = l->forecast_zone;
    if (!radar && l->radar) radar = l->radar;
    if (!tz_hint && l->tz_hint) tz_hint = l->tz_hint;
    l = l->parent;
  }

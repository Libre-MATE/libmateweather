...
char *mateweather_parser_get_value(MateWeatherParser *parser) {
  ...
  /* check for null node */
  if (xmlTextReaderIsEmptyElement(parser->xml)) return NULL;

  /* the next "node" is the text node containing the value we want to get */
  if (xmlTextReaderRead(parser->xml) != 1) return NULL;
...
MateWeatherParser *mateweather_parser_new(gboolean use_regions) {
  ...
  for (i = 0; parser->locales[i] != NULL; i++) {
    ...
    if (g_file_test(filename, G_FILE_TEST_IS_REGULAR)) break;

    g_free(filename);
    filename = NULL;

    if (!zlib_support) continue;
  ...
  if (parser->xml == NULL) goto error_out;
...
void mateweather_parser_free(MateWeatherParser *parser) {
  if (parser->xml) xmlFreeTextReader(parser->xml);

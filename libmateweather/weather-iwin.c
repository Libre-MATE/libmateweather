...
static GSList *parseForecastXml(const char *buff, WeatherInfo *master_info) {
  GSList *res = NULL;
  xmlDocPtr doc;
  xmlNode *root, *node;
...
#define isElem(_node, _name) g_str_equal((const char *)_node->name, _name)
  ...
  for (node = root->xmlChildrenNode; node; node = node->next) {
    if (node->name == NULL || node->type != XML_ELEMENT_NODE) continue;
        ...
        if (isElem(n, "time-layout")) {
          ...
          for (p = n->children; p; p = p->next) {
            if (p->name && isElem(p, "temperature") &&
              ...
              if (!is_max && !hasAttr(p, "type", "minimum")) break;
          ...
          if (res) {
            ...
            for (r = res; r; r = r->next) {
              ...
              if (!nfo || !nfo->valid) {
                if (r->data) weather_info_free(r->data);
...
static void iwin_finish(SoupSession *session, SoupMessage *msg, gpointer data) {
  ...
  if (info->forecast_type == FORECAST_LIST) {
    ...
    if (loc->latlon_valid) {
      ...
      url = g_strdup_printf(
          ...
          "browser_interface/"
          "ndfdBrowserClientByDay.php?&lat=%.02f&lon=%.02f&format=24+hourly&"
          "startDate=%04d-%02d-%02d&numDays=7",
          RADIANS_TO_DEGREES(loc->latitude), RADIANS_TO_DEGREES(loc->longitude),
          year, month, day);
